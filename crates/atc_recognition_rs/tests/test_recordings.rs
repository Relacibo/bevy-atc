...
static SPEECH_TO_TEXT: LazyLock<SpeechToText> = LazyLock::new(|| {
    let config = SpeechToTextConfig {
        model_path: std::path::Path::new(concat!(
...
mod audio_recognition_tests {
    ...
    fn test_audio_recognition_for_aircraft(aircraft_key: &str) {
        let TestRecording {
            strings: text, file_name, ..
        } = &TEST_RECORDINGS_INDEX.entries[aircraft_key];
        let wav_path = resolve_test_recording_file(file_name);
        ...
        assert!(
            text.contains(&transcribed_text),
            "Expected one of: {text:?}, got {transcribed_text}"
        )
    }
...
mod text_parsing_tests {
    ...
    fn test_text_parsing_for_aircraft(aircraft_key: &str) {
        ...
        for expected_text in expected_strings {
